...
<Mermaid
  caption="webhookd — synchronous dispatch with retries"
  chart={`flowchart LR
    V[Visitor] -->|target URL + payload| API[Send route]
    API -->|rate limit demo:webhookd| RL[(rate_limit_events)]
    API -->|validateExternalUrl| URL[lib/url-validation]
    API -->|sign HMAC-SHA256| S[lib/webhookd:sign]
    S -->|attempt 1| T[Target URL]
    T -.->|fail| API
    API -->|backoff + jitter| S
    S -->|attempt 2| T
    T -.->|2xx| API
    API -->|attempts log + secret| V
`} />

## Canonical Go architecture

<Mermaid
  caption="webhookd Go service — async durable delivery"
  chart={`flowchart LR
    App[Application] -->|EnqueueDelivery| DB[(Postgres<br/>deliveries)]
    Worker[Dispatcher worker] -->|FOR UPDATE SKIP LOCKED| DB
    Worker -->|sign + POST| Customer[Customer endpoint]
    Customer -.->|2xx| Worker
    Worker -->|RecordAttempt<br/>+ schedule next| DB
    Worker -->|decorrelated jitter retry| Worker
`} />
